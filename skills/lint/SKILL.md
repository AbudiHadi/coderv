...
| 🟡 **Rot** | Draft files with STALE banners older than the last real doc update; orphan docs; empty sections; KNOWN-ISSUES prevention rules that never landed in CLAUDE.md or a test. An effort map (`docs/PLAN-*.md`) still `Status: active` but untouched (git) for >30 days — the effort stalled or the map lies; also flag any "Decisions so far" line that restates decision content instead of linking an ADR (the map is an index, never a store). |
| 🟠 **Stale vocabulary** | `docs/CONTEXT.md` terms, checked deterministically: every row in the **Terms** table has a code anchor — a `file:line` citation (verify like any other citation: file exists, line in range) or a grep-able identifier (search the row's **Code anchor** value, fixed-string: `grep -rFln --exclude-dir={docs,.git,node_modules,dist,build} -- '<anchor>' .` must hit ≥1 file — the term itself is only for reporting, and the glossary's own row can never be its anchor, so `docs/` is excluded mechanically, not by judgment). Anchor dead → flag the term. The **Domain-only terms** section is skipped by design (no anchors there) — check it only for terms duplicated between the two tables. No fuzzy "seems unused" judgment: anchor resolves = fresh, anchor dead = flagged, nothing in between. |
